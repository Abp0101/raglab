...
This experiment measures complete query behavior, not isolated framework overhead. LangGraph may make a second local model call after invalid citations, which is recorded in usage. The experiment does not establish a universal framework ranking or isolate model nondeterminism. Framework-specific splitters and vector indexes are evaluated separately by the [native indexing experiment](indexing-experiments.md), so their changes cannot silently contaminate this canonical baseline. The shared data-plane rationale is recorded in [ADR 0001](adr/0001-shared-canonical-retrieval-for-framework-adapters.md), the bounded graph decision in [ADR 0002](adr/0002-bounded-langgraph-citation-repair.md), the LlamaIndex mapping in [ADR 0003](adr/0003-map-canonical-retrieval-to-llamaindex-nodes.md), and the Haystack graph in [ADR 0004](adr/0004-use-haystack-async-components-over-shared-retrieval.md).
